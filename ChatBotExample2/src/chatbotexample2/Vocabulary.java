...
public class Vocabulary implements ActionListener{
    ...
    public Vocabulary() {
        ...
        Frame = new JFrame("OntBot Vocabulary");
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getActionCommand().equalsIgnoreCase("Add Key")) {
            String key = JOptionPane.showInputDialog(Frame, "Please enter the key: ");
            addKey(key);
        } else if (ae.getActionCommand().equalsIgnoreCase("Remove Key")) {
            removeKey();
        } else if (ae.getActionCommand().equalsIgnoreCase("Add Phrase")) {
            String phrase = JOptionPane.showInputDialog(Frame, "Please enter the phrase: ");
            addPhrase(phrase);
    ...
    public void reset() {
        ...
        for (int i = 0; i < phrases.size(); i++) {
            TabbedPane.addTab(phrases.get(i).getKey(),phrases.get(i).getJListPhrases());
        }
    }
    //Create a show method to show the JFrame
    public void show(boolean showMe) {
       if (showMe) Frame.setVisible(true);
    ...
    //The searchPhrases method - searches the PhraseCollections for the userInput
     ...
     public void addKey(String key) {
        ...
        if (searchForKey(key) == -1) {
            ...
            TabbedPane.addTab(key, pc.getJListPhrases());
